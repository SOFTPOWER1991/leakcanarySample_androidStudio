  ...
  static class Calculator {
    ...
    public void compute()
      ...
      snapshot.getIndexManager()
          .setReader(Index.DOMINATOR, new IndexWriter.IntIndexStreamer().writeTo(
              Index.DOMINATOR.getFile(snapshot.getSnapshotInfo().getPrefix()),
              new IteratorInt() {
                int nextIndex = 2;
    ...
    private void writeIndexFiles(FlatDominatorTree tree) throws IOException {
      ...
      IndexWriter.IntArray1NWriter writer = new IndexWriter.IntArray1NWriter(dom.length - 1,
          Index.DOMINATED.getFile(snapshot.getSnapshotInfo().getPrefix()));
      ...
      snapshot.getIndexManager().setReader(Index.DOMINATED, writer.flush());
      ...
      public void calculateTotalSizesIterative(int e) throws SnapshotException, IOException {
        ...
        dump.getIndexManager()
            .setReader(Index.O2RETAINED, retained.writeTo(
                Index.O2RETAINED.getFile(dump.getSnapshotInfo().getPrefix())));
        retained = null;
